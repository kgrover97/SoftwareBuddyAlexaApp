...
export default class Proxy {
    ...
    private async hanshakeHandle(req: restify.Request, res: restify.Response, next: restify.Next) {
        Log.info("Recieved a request to /");
    }
    ...
    private async alexaMessageHandler(req: restify.Request, res: restify.Response, next: restify.Next) {
        try {
            ...
            let timedOut = false;
            // let messageHandler: MessageEvent = function (response: any) {
            ...
            if (await this._wss.isClientReady(user)) {
                ...
                Log.info("Sending req to client");
